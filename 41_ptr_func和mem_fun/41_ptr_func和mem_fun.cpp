...
public:
	...
	void printInfo1(int ii) { cout << m_Id << ii << endl; }
private:
	int m_Id;
	static int m_wId;
};
...
int main()
{
	...
	//for_each(v1.begin(), v1.end(), mem_fun_ref(&Widget::printInfo1)); //不能传递参数，所以真的不好用,不如用lambda表达式
	...
	//lambda表达式都可以很容易的解决,而且mem_fun不能传递参数
